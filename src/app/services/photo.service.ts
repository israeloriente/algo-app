...
export class PhotoAction {
  ...
  constructor(private imageCompress: NgxImageCompressService, public photo: Photo, public bucketName: string, action: "add" | "remove", public check: CheckController) {
    this.action = action;
  }
  ...
  public async compress(): Promise<boolean> {
    ...
    try {
      if (!this.isCompression && this.photo.base64.length > PhotoAction.MAX_MEGABYTE * 1024) {
        ...
        return await this.imageCompress.compressFile(imgeBase64, -1, 70, 50).then(
          (result) => {
  ...
  private async checkComplete() {
    ...
    if (complete) {
      ...
      if (fail.length > 0 && this.check.attempts <= PhotoAction.MAX_ATTEMPTS) {
        ...
        this.check.service.Photo = this.check.actions.filter((e) => e.url != undefined && e.status == "success").map((e) => { return { Url: e.url } }) || [];
        ...
        await this.check.service.update().finally(() => {
          ...
          if (fails.length > 0) {
            ...
            new Report(
              { message: `Error to send images: ${fails.length}` },
              "class.services.PhotoAction.checkComplete",
              false
            );
    ...
    try {
      if (!this.photo.objectKey.includes(this.bucketName)) {
        ...
        await AWSS3Service.uploadImageBase64(
          this.bucketName,
          `${this.check.service.className}-${this.check.service.Id}/${date.getFullYear()}-${date.getMonth() + 1}-${date.getDate()}/${this.photo.objectKey}`,
          this.photo.base64
  ...
  public static async create(imageCompress: NgxImageCompressService, service: Service, bucketName: string, imagesAdd: Photo[], imagesRemove: Photo[] = []): Promise<PhotoAction[]> {
